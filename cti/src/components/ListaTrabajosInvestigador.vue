  ...
  <div class="user-data">
    ...
    <div class="condecoraciones">
      <router-link v-for="premio in filteredPremios" :key="premio.id" :to="{ path: getCategoryPath('premio', premio.id) }"
        ...
        <div class="condecoracion-nombre">{{ premio.denominacion }}</div>
...
export default {
  ...
  methods: {
    async obtenerArticulosUsuario() {
      try {
        const investigatorId = this.$route.params.id; // Obtén el ID del investigador de la ruta
        //busca los articulo a partir del id del investigador seleccionado
        const respuesta = await axios.get(`http://127.0.0.1:8000/api/v1.0/usuario/${investigatorId}/write_articulo/`);
        
        this.articulosUsuario = respuesta.data;
        console.log('los articulos :',this.articulosUsuario)
      } catch (error) {
    ...
    async obtenerLibrosUsuario() {
      try {
        const investigatorId = this.$route.params.id; // Obtén el ID del investigador de la ruta
        //busca los libro a partir del id del investigador seleccionado
        const respuesta = await axios.get(`http://127.0.0.1:8000/api/v1.0/usuario/${investigatorId}/write_libro/`);
        console.log('los libro :',respuesta.data)
    ...
    async obtenerMonografiasUsuario() {
      try {
        const investigatorId = this.$route.params.id; // Obtén el ID del investigador de la ruta
        //busca los monografia a partir del id del investigador seleccionado
        const respuesta = await axios.get(`http://127.0.0.1:8000/api/v1.0/usuario/${investigatorId}/write_monografia/`);
        console.log('los monografias :',respuesta.data)
    ...
    async obtenerPremiosUsuario() {
      try {
        const investigatorId = this.$route.params.id; // Obtén el ID del investigador de la ruta
        //busca los premios a partir del id del investigador seleccionado
        const respuesta = await axios.get(`http://127.0.0.1:8000/api/v1.0/usuario/${investigatorId}/write_premio/`);
        console.log('los premio :',respuesta.data)
    ...
    async obtenerReconocimientosUsuario() {
      try {
        const investigatorId = this.$route.params.id; // Obtén el ID del investigador de la ruta
        //busca los reconocimiento a partir del id del investigador seleccionado
        const respuesta = await axios.get(`http://127.0.0.1:8000/api/v1.0/usuario/${investigatorId}/write_reconocimiento/`);
        console.log('los reconocimineto :',respuesta.data)
